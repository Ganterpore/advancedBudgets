<script>
    ...
    return debts
      .map(d => {
        for (let y = 0; y <= yearsUntil; y++) {
          if (compoundedValue(d.currentBalance - d.total, d.payPerPeriod, d.percent, 12, y) >= 0) {
            return {
              year: y,
  ...
  function retirementDataInYears (year) {
    ...
    const debtRemaining = debts
      .map(d => Math.min(0, compoundedValue(d.currentBalance - d.total, d.payPerPeriod, d.percent, 12, year)))
